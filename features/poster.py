...
async def _find_poster_from_tmdb(query: str, year: str = None):
    ...
    params_attempts = []
    if year:
        params_attempts.append({"api_key": Config.TMDB_API_KEY, "query": query, "include_adult": "false", "year": str(year)})
    params_attempts.append({"api_key": Config.TMDB_API_KEY, "query": query, "include_adult": "false"})
        ...
        async with aiohttp.ClientSession() as session:
            for params in params_attempts:
                async with session.get(search_url, params=params, timeout=10) as resp:
                    ...
                    # 1. Exact Match Check (Best for short titles like "Road")
                        ...
                        if is_exact:
                            if year and result_year:
                                try:
                                    ...
                                    pass
                            if orig_lang in ['hi', 'mr', 'ta', 'te', 'ml', 'kn', 'en']:
                                return f"https://image.tmdb.org/t/p/original{poster}"

                    # 2. Fuzzy Match Check (Only for multi-word titles like shows)
                    if not is_single_word:
                        for result in results[:8]:
                            ...
                            score = max(fuzz.ratio(query_norm, title_match), fuzz.ratio(query_norm, original_title)) if fuzz else 0

                            if score >= 65 or query_norm in title_match or title_match in query_norm:
                                if year and result_year:
                                    try:
                                        if abs(int(year) - int(result_year)) > 1:
                                            continue
