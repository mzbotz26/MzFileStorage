...
def generate_search_queries(title: str):
    ...
    # Comedycha 5G jaise subtitle tags search query se hatayein
...
async def _find_poster_from_tmdb(query: str, year: str = None):
    ...
    try:
        ...
        # Search runs: 1. With year (if available), 2. Without year fallback
        attempts = []
        if year:
            ...
            for params in attempts:
                async with session.get(search_url, params=params, timeout=10) as resp:
                    ...
                    for result in results[:10]:
                        ...
                        # Single-word rule: strictly reject partial words (e.g. Road to Perdition)
                        if is_single_word and not is_exact:
                            continue

                        # Year check when available
                        year_ok = True
                        ...
                        # Indian language priority for exact matches
                        if is_exact:
                            if year and year_ok:
                                if result.get("poster_path"):
                                    return f"https://image.tmdb.org/t/p/original{result['poster_path']}"
                            elif not year or orig_lang in ['hi', 'mr', 'ta', 'te', 'ml', 'kn']:
                                if result.get("poster_path"):
                                    return f"https://image.tmdb.org/t/p/original{result['poster_path']}"

                        # Multi-word fuzzy check (Shows like Maharashtrachi Hasyajatra)
                        if not is_single_word:
                            ...
                            if (best_score >= 60 or query_norm in title_match or title_match in query_norm) and year_ok:
                                if result.get("poster_path"):
                                    return f"https://image.tmdb.org/t/p/original{result['poster_path']}"
