...
def generate_search_queries(title: str):
    ...
    # Space variations for Indian regional shows
    if "hasyajatra" in clean_title.lower():
        v = re.sub(r'(?i)hasyajatra', 'Hasya Jatra', clean_title).strip()
        if v not in queries:
            queries.append(v)
    elif "hasya jatra" in clean_title.lower():
        v = re.sub(r'(?i)hasya jatra', 'Hasyajatra', clean_title).strip()
        if v not in queries:
            queries.append(v)
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(search_url, timeout=10) as resp:
                ...
                for item in result_items[:5]:
                    ...
                    if score < 55 and query_norm not in found_title and found_title not in query_norm:
                        continue
