        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(search_url, params=params, timeout=10) as resp:
                ...
                for result in results[:10]:
                    ...
                    # 1. Chote single-word titles (jaise "Road") ke liye exact title match zaroori hai
                    if is_single_word:
                        if query_norm != title_match and query_norm != original_title:
                            continue

                    # 2. Year check (agar saal diya gaya ho)
                    ...
                    # 3. Fuzzy & substring match for multi-word titles / shows
                    ...
                    if (
                        query_norm == title_match 
                        or query_norm == original_title 
                        or best_score >= 60 
                        or query_norm in title_match 
                        or title_match in query_norm
                    ):
