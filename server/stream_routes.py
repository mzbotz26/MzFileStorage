...
# In-Memory Active Sessions & Burned Token Tracking
ACTIVE_SESSIONS = {}
USED_TOKENS = set()

def register_short_session(short_id, owner_id, file_id, user_id, ts):
    current_t = int(time.time())
    expired = [k for k, v in ACTIVE_SESSIONS.items() if current_t - v.get("ts", 0) > 600]
    for k in expired:
...
<body>
    <div class="card">
        ...
        <div class="alert-box">⚠️ Security violation!<br>You must open and complete the shortener manually in a browser.</div>
...
<body>
    ...
    <script>
        ...
        // Check if opened directly without shortener redirect
        let ref = document.referrer.toLowerCase();
        if (!ref || ref.includes("telegram") || ref.includes("t.me")) {{
...
# =====================================================================
# 🔒 SHORT LINK DESTINATION (e.g., /v/alXAH9B?sig=...)
# =====================================================================
...
async def short_destination_verify_handler(request):
    ...
    # 🚨 1. SERVER-SIDE REFERER ENFORCEMENT
    # Agar user Telegram ya direct paste se aayega toh block hoga
    http_ref = request.headers.get("Referer", "").lower()
    if http_ref and ("t.me" in http_ref or "telegram" in http_ref):
        return web.Response(text=ACCESS_DENIED_HTML, content_type="text/html", status=403)
    ...
    # 🚨 5. Generate Dynamic Challenge Token
...
async def api_v5_confirm_handler(request):
    try:
        ...
        # 🚨 REFERER VALIDATION: Direct paste without shortener redirect is BLOCKED
        if not client_ref or "telegram" in client_ref or "t.me" in client_ref:
            return web.json_response({"status": "error", "message": "Referer validation failed"}, status=403)
        ...
        # 🚨 TIME WINDOW CHECK: 20s to 75s
        if elapsed < 20 or elapsed > 75:
        ...
        # 🔥 Instantly Burn Token & Destroy Session
