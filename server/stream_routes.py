...
async def api_v5_confirm_handler(request):
    try:
        ...
        # --- STEP LOG DISPATCH FUNCTION ---
        async def send_channel_log(step_num, is_final=False):
            if not vlog_ch or not bot:
                return
            try:
                status_header = "🎉 **Final Verification Complete!**" if is_final else f"⚡️ **Shortener Step {step_num} Passed!**"
                log_msg = (
                    f"{status_header}\n\n"
        ...
        # Check: Intermediate Step vs Final Step
        if current_step < total_steps:
            await send_channel_log(current_step, is_final=False)
            await set_user_verify_step(owner_id, requester_id, current_step + 1)
        else:
            await send_channel_log(total_steps, is_final=True)
