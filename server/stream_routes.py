...
<body>
    <div class="card">
        ...
        <div class="alert-box">⚠️ ⚠️ Bypass Bot Detected!<br>Direct bypass links are strictly blocked.</div>
        <div class="footer">🔒 Security Enforced by <b>Av Bypass Bot ⚡</b></div>
    </div>
</body>
</html>"""

INTERACTIVE_CHECK_HTML = """<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Verifying Integrity...</title>
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        body {{
...
<body>
    <div class="card" id="mainCard">
        <h2 style="color: #38bdf8;">🛡️ Security Check</h2>
        <p style="color: #94a3b8; font-size: 14px; margin-top: 8px;" id="statusText">Validating browser session...</p>
        <div class="spinner" id="loader"></div>
        <button class="btn" id="claimBtn" onclick="claimAccess()">🚀 Get Telegram File</button>
    </div>
    ...
    <script>
        ...
        setTimeout(() => {{
            ...
            document.getElementById('statusText').innerText = '✅ Session verified! Click below to unlock:';
            document.getElementById('claimBtn').style.display = 'block';
        }}, 3000);

        async function claimAccess() {{
            let clientDuration = Math.floor((Date.now() - clientStartTime) / 1000);
            document.getElementById('claimBtn').innerText = 'Unlocking...';
            document.getElementById('claimBtn').disabled = true;

            try {{
                let resp = await fetch("/api/v2/confirm", {{
                    method: "POST",
                ...
                if (data.status === "ok") {{
                    ...
                    document.body.innerHTML = `{access_denied_raw}`;
                }}
            }} catch (e) {{
                document.body.innerHTML = `{access_denied_raw}`;
            }}
...
# =====================================================================
# 🔒 2-STEP BROWSER HANDSHAKE GATEWAY (BYPASS BOT IMMUNE)
# =====================================================================
...
async def shortener_destination_verify_handler(request):
    ...
    # Dynamic One-Time Nonce generate karein
    nonce = hashlib.md5(f"{payload}_{ts}_{secret_key}".encode()).hexdigest()

    safe_denied = ACCESS_DENIED_HTML.replace("`", "'").replace("\n", "")

    html = INTERACTIVE_CHECK_HTML.format(
        payload=payload,
        sig=sig_param,
        nonce=nonce,
        access_denied_raw=safe_denied
    )
    return web.Response(text=html, content_type="text/html")


@routes.post("/api/v2/confirm")
async def api_v2_confirm_handler(request):
    try:
        ...
        if not sig_param or "___" not in sig_param or duration < 2:
        ...
        # 1. Nonce Verify
        expected_nonce = hashlib.md5(f"{payload}_{ts}_{secret_key}".encode()).hexdigest()
        ...
        # ✅ Real Browser Confirmed -> Database Verification Mark
        ...
        logger.error(f"Error in api_v2_confirm: {e}")
