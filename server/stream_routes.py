...
<body>
    <div class="card">
        ...
        <div class="alert-box">⚠️ ⚠️ Bypass Bot Detected!<br>Please open and complete the shortener link in a real browser.</div>
...
<body>
    <div class="card">
        ...
        <p>Shortener verification completed.<br>Your access is active for <b>24 Hours</b>.</p>
        <a href="{bot_url}" class="btn">🚀 Open in Telegram</a>
    </div>
    <script>
        setTimeout(() => {{ window.location.href = "{bot_url}"; }}, 1500);
    </script>
</body>
</html>"""
...
# =====================================================================
# 🔒 STRICT SHORTENER DESTINATION (BLOCKS BYPASS BOT AUTOMATICALLY)
# =====================================================================
...
async def shortener_destination_verify_handler(request):
    ...
    # 1. User-Agent / Referer Check (Bypass Bots do not send browser headers)
    referer = request.headers.get("Referer", "").lower()
    user_agent = request.headers.get("User-Agent", "").lower()
    
    # Agar User-Agent Python/Aiohttp/Bot ka hai -> Direct Block
    if any(b in user_agent for b in ["python", "aiohttp", "bot", "curl", "requests", "go-http"]):
        return web.Response(text=ACCESS_DENIED_HTML, content_type="text/html", status=403)

    # 2. Minimum Time Limit (Shortener solve karne me min 30 seconds lagte hain)
    current_time = int(time.time())
    elapsed = current_time - ts
    link_expiry = getattr(Config, "LINK_EXPIRY", 900)

    # Agar 30 sec se kam me link open hua -> 100% Bypass Bot -> Block!
    if elapsed < 30 or elapsed > link_expiry:
        return web.Response(text=ACCESS_DENIED_HTML, content_type="text/html", status=403)
    ...
    # ✅ Real User Shortener Se Aaya -> Database me Unlock karein
