...
# 🚨 In-Memory Single Use Tracker (Bypass Bot link touch karte hi burn ho jayegi)
USED_NONCES = set()
...
<body>
    <div class="card">
        ...
        <div class="alert-box">⚠️ Bypass Bot Detected!<br>Direct bypass links are strictly blocked.<br>Please open and complete the shortener manually.</div>
...
<body>
    ...
    <script>
        ...
        async function verifyBrowser() {{
            ...
            try {{
                let resp = await fetch("/api/v3/confirm_token", {{
                    method: "POST",
...
# =====================================================================
# 🔒 DESTINATION GATEWAY (DIRECT SHORTENER RETURN)
# =====================================================================
...
async def shortener_destination_verify_handler(request):
    ...
    # 3. Dynamic Interactive Token
    token = hashlib.sha256(f"{payload}_{ts}_{secret_key}_mz".encode()).hexdigest()
    
    # 4. Check if already burned by bot scraper
    if token in USED_NONCES:
        return web.Response(text=ACCESS_DENIED_HTML, content_type="text/html", status=403)
...
@routes.post("/api/v3/confirm_token")
async def api_v3_confirm_handler(request):
    try:
        ...
        # Real user browser must spend at least 2 seconds on challenge
        if not sig_param or "___" not in sig_param or spent < 2:
            return web.json_response({"status": "error"}, status=403)

        # 🚨 SINGLE-USE NONCE BURN CHECK
        if token in USED_NONCES:
            return web.json_response({"status": "error", "message": "Token already consumed/bypassed"}, status=403)
        ...
        # 🚨 TIME LOCK: Real ad solving takes 25s - 120s max.
        # Bypass bots (<25s) or Delayed bypass links (>120s) are permanently BLOCKED.
        if elapsed < 25 or elapsed > 120:
            return web.json_response({"status": "error", "message": "Time verification failed"}, status=403)

        secret_key = getattr(Config, "SECRET_KEY", "mz_super_secret_anti_bypass_key_2026")

        # 1. Token validation
        expected_token = hashlib.sha256(f"{payload}_{ts}_{secret_key}_mz".encode()).hexdigest()
        ...
        # 🔥 Instantly burn token so it cannot be reused
        USED_NONCES.add(token)

        # Memory optimization: Keep set size manageable
        ...
        logger.error(f"Error in api_v3_confirm: {e}")
