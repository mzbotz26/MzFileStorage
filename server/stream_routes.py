...
<body>
    <div class="card">
        ...
        <div class="footer">🔒 Security Enforced by <a href="https://t.me/MzFileStorage_Bot" target="_blank" rel="noopener noreferrer">𝐌𝐳 𝐅𝐢𝐥𝐞 𝐒𝐭𝐨𝐫𝐞</a> ⚡</div>
    </div>
</body>
</html>"""
...
<body>
    <div class="card">
        ...
        <p style="color: #94a3b8; font-size: 14px; margin-top: 8px;" id="msg">Verifying shortener impression integrity...</p>
...
# =====================================================================
# 🔒 SHORT LINK DESTINATION (e.g., /v/MJrtzXG?sig=...)
# =====================================================================
...
async def short_destination_verify_handler(request):
    ...
    # 1. Block direct Telegram app clicks
    http_ref = request.headers.get("Referer", "").lower()
    if http_ref and ("t.me" in http_ref or "telegram" in http_ref or "org.telegram" in http_ref):
        return web.Response(text=ACCESS_DENIED_HTML, content_type="text/html", status=403)
...
async def api_v5_confirm_handler(request):
    try:
        ...
        # Real user browser must spend at least 1s on page
        if not sig_param or "___" not in sig_param or spent < 1:
        ...
        # Real shortener time: 10s to 600s
        if elapsed < 10 or elapsed > 600:
        ...
        # Specific Step Gap from owner settings (e.g. verify_gap_1, verify_gap_2, verify_gap_3)
        if owner_settings:
            gap_mins = owner_settings.get(f'verify_gap_{current_step}') or owner_settings.get('verify_gap', 720)
        else:
            gap_mins = 720

        # File & Channel Info for Log
        ...
        # Incremental Step Verification: User gets access for gap_mins immediately
        await claim_step_verification(owner_id, file_unique_id, requester_id, current_step, total_steps)

        bot_username = bot.me.username if bot and hasattr(bot, 'me') and bot.me else "MzFileStorageBot"
