...
async def add_user(user_id):
    ...
    user_data = {
        ...
        # Verification Gap for each step (Minutes)
        'verify_gap_1': 30,
        'verify_gap_2': 60,
        'verify_gap_3': 720,
        'verify_gap': 720,  # Global fallback
...
async def add_verified_user(owner_id: int, requester_id: int, gap_mins: int = 720):
    """Marks the user as verified with current timestamp and expiry."""
    try:
        now = datetime.datetime.utcnow()
        expires_at = now + datetime.timedelta(minutes=int(gap_mins))
        await verified_users.update_one(
            {
                "owner_id": owner_id,
...
async def claim_step_verification(owner_id: int, file_unique_id: str, requester_id: int, current_step: int, total_steps: int, gap_mins: int = None) -> bool:
    """
    User ko current step ke baad access deta hai,
    exact expiry time calculate karke save karta hai aur agle cycle ke liye next_step set karta hai.
    """
    ...
    owner_data = await get_user(owner_id) or {}
    if gap_mins is None:
        gap_val = owner_data.get(f'verify_gap_{current_step}') or owner_data.get('verify_gap', 720)
        try:
            gap_mins = int(gap_val)
        except Exception:
            gap_mins = 720
...
async def is_user_verified(owner_id: int, requester_id: int) -> bool:
    """
    Checks if user is currently within their active verification time gap.
    """
    try:
        ...
        # 1. Direct check with expires_at (Most reliable)
        if "expires_at" in verification and isinstance(verification["expires_at"], datetime.datetime):
            return verification["expires_at"] > now

        # 2. Fallback check with verified_at
        if "verified_at" in verification and isinstance(verification["verified_at"], datetime.datetime):
            owner_data = await get_user(owner_id) or {}
            completed_step = verification.get("completed_step", 1)
            gap_val = owner_data.get(f'verify_gap_{completed_step}') or owner_data.get('verify_gap', 720)
            try:
                gap_minutes = int(gap_val)
            except Exception:
                gap_minutes = 720

            expiry_threshold = now - datetime.timedelta(minutes=gap_minutes)
...
async def claim_verification_for_file(owner_id: int, file_unique_id: str, requester_id: int) -> bool:
    ...
    owner_data = await get_user(owner_id) or {}
    gap_val = owner_data.get('verify_gap', 720)
    try:
        gap_mins = int(gap_val)
    except Exception:
        gap_mins = 720

    await add_verified_user(owner_id, requester_id, gap_mins=gap_mins)
    await record_daily_view(owner_id, requester_id)
    return True
