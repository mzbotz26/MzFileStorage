...
async def claim_step_verification(owner_id: int, file_unique_id: str, requester_id: int, current_step: int, total_steps: int) -> bool:
    ...
    completed_step save karta hai aur agle cycle ke liye next_step set karta hai.
    """
...
async def is_user_verified(owner_id: int, requester_id: int) -> bool:
    """
    Checks if user is verified within the completed step's verify_gap.
    """
    try:
        ...
        # User ne jo step complete kiya tha uska gap uthana
        completed_step = verification.get("completed_step", 1)
        gap_minutes = owner_data.get(f'verify_gap_{completed_step}') or owner_data.get('verify_gap', 720)
