...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # Non-breaking spaces (\xa0) aur extra spaces ko normalize karein
    clean_raw_name = name.replace('\xa0', ' ').replace('\u200b', ' ')

    # Original filename se reliable 4-digit release year pakadein
    extracted_year = extract_year_from_filename(clean_raw_name)

    # Unicode normalization: stylized letters ko standard text me convert karna
    ...
    standalone_s = re.search(r'\b(?:S|Season)\s*0*(\d{1,3})\b', name_for_parsing, re.IGNORECASE)
    if standalone_s:
        season_info_str = f"S{int(standalone_s.group(1)):02d}"

    # --- 1. RANGE CHECK (HANDLES: [E01 - E05], [E06 - E09], Ep 01-04, 01-04, 1 to 4) ---
    range_patterns = [
        (r'\[\s*(?:EP?|E)?\s*0*(\d{1,4})\s*(?:[-–—toTo]|\s+)\s*(?:EP?|E)?\s*0*(\d{1,4})\s*\]', 'no_season'),
        (r'\b(?:Ep|Episode|Epi|E)\s*0*(\d{1,3})\s*(?:[-–—toTo]|\s+)\s*0*(\d{1,3})\b', 'no_season'),
        (r'S(\d{1,3}).*?\[\s*E?(\d{1,4})\s*(?:-|–|—|to)\s*E?(\d{1,4})\s*\]', 'season'),
    ...
    # --- 2. AGAR RANGE NAHI MILI, TABHI SINGLE EPISODE DHOONDEIN ---
    ...
    # Clean brackets and episode markers BEFORE PTN parsing to prevent leaks
    ...
    if not episode_info_str:
        ...
        if ep_val:
            if isinstance(ep_val, list):
                ...
                elif ep_val: episode_info_str = f"EP {int(ep_val[0])}"
            else:
                try:
                    episode_info_str = f"EP {int(ep_val)}"
                except Exception:
    ...
    # Title ke andar se bache hue isolated numbers saaf karein
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    for info in media_info_list:
        ...
        if info.get('episode_info'):
            ep_val = str(info['episode_info']).strip()
            if "-" in ep_val:
                numbers = re.findall(r'\d+', ep_val)
                if len(numbers) >= 2:
