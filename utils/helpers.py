...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # DS4K aur 4K ko pehle clean karein taaki 'S4' fake season na ban sake
    clean_for_season = re.sub(r'\bDS-?4K\b|\b4K\b', ' ', clean_name_ascii.replace('_', ' '), flags=re.IGNORECASE)

    # Standalone Season check (e.g. S01, S1, Season 1) - Negative lookbehind se ensure karein ki word ke beech ka 'S' match na ho
