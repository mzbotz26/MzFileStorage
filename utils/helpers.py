...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # Standalone Season check (e.g. S01, S1, Season 1)
    standalone_s = re.search(r'\b(?:S|Season)\s*0*(\d{1,3})\b', clean_name_ascii, re.IGNORECASE)
    if standalone_s:
        season_info_str = f"S{int(standalone_s.group(1)):02d}"

    # ============================================================
    # 🎯 BULLETPROOF EPISODE RANGE EXTRACTION
    # ============================================================
    range_match = re.search(
        r'\[\s*(?:EP?|E)?\s*0*(\d{1,4})\s*(?:[-–—toTo]|\s+)\s*(?:EP?|E)?\s*0*(\d{1,4})\s*\]',
        clean_raw_name,
        re.IGNORECASE
    )
    if range_match:
        s_ep, e_ep = int(range_match.group(1)), int(range_match.group(2))
        if s_ep < e_ep and e_ep < 500:
            episode_info_str = f"EP {s_ep:02d}-{e_ep:02d}"

    if not episode_info_str:
        range_match_2 = re.search(
            r'\b(?:Ep|Episode|Epi|E)?\s*0*(\d{1,3})\s*(?:[-–—]|to)\s*(?:Ep|Episode|Epi|E)?\s*0*(\d{1,3})\b',
            clean_name_ascii,
            re.IGNORECASE
        )
        if range_match_2:
            s_ep, e_ep = int(range_match_2.group(1)), int(range_match_2.group(2))
            if s_ep < e_ep and e_ep < 500:
                episode_info_str = f"EP {s_ep:02d}-{e_ep:02d}"

    if not episode_info_str:
        single_match = re.search(r'\b(?:EP?|Episode)\s*0*(\d{1,4})\b', clean_name_ascii, re.IGNORECASE)
        if single_match:
            ep_num = int(single_match.group(1))
            if ep_num < 1000:
                episode_info_str = f"EP {ep_num}"

    # Clean markers for PTN parsing
    ...
    final_title = cleaned_title.title()
    # Remove any stray year string from final title
    final_title = re.sub(r'[\(\[]\s*(?:19\d{2}|20\d{2})\s*[\)\]]', ' ', final_title)
    ...
    # ============================================================
    # 🔒 STRICT SINGLE-YEAR RESOLUTION (NO DUPLICATES)
    # ============================================================
    ...
    if final_year:
        ...
        # Ensure year isn't already inside the string before adding
        display_title_main = re.sub(rf'[\(\[]?\s*{year_str}\s*[\)\]]?', '', display_title_main).strip()
