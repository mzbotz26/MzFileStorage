...
async def get_definitive_title_from_imdb(title_from_filename):
    ...
    try:
        ...
        similarity = fuzz.ratio(normalized_original, normalized_imdb) if fuzz else 0
        ...
        if title_from_filename.lower() not in imdb_title.lower() and imdb_title.lower() not in title_from_filename.lower():
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    for info in media_info_list:
        ...
        # Quality & Rip handling (e.g., 576p | WEB-DL | H.264)
        if info.get('quality_tags'):
            display_tags_parts.append(info['quality_tags'])

        # Language handling: separated by ' | '
        languages = info.get('languages', [])
        if languages:
            display_tags_parts.append(" | ".join(languages))
        
        # Combine all parts with vertical bar divider
...
def calculate_title_similarity(title1: str, title2: str) -> float:
    ...
    return fuzz.token_sort_ratio(title1.lower(), title2.lower()) if fuzz else 0
...
async def get_main_menu(user_id):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("🖼️ IMDb Poster", callback_data="poster_menu"), InlineKeyboardButton("📂 My Files", callback_data="my_fil
