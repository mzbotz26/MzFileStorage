...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # 🛑 DS4K / 4K tags ko clean karein taaki 'S4' fake season na ban sake
    ...
    # Standalone Season: 'S01' ya 'Season 1'
    ...
    # PTN Fallback: Tabhi accept karein jab filename me actually "Season X" ya "SX" present ho
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    # 🔒 Batch-wide year consistency: Agar kisi bhi file me year hai toh poore batch ka header lock karein
    batch_year = None
    ...
    clean_base = re.sub(r'\s*\((?:19\d{2}\vert{}20\d{2})\)', '', first_info['display_title']).strip()
    if batch_year:
        primary_display_title = f"{clean_base} ({batch_year})"
    else:
