...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    if not episode_info_str:
        ...
        if ep_val:
            ...
            # Fallback regex for direct E4, EP04 in name
            ep_direct = re.search(r'\b(?:E|EP)\s*0*(\d{1,4})\b', name_for_parsing, re.IGNORECASE)
            if ep_direct:
    ...
    # Reality shows me Day / Tagline ke aage ka sab text cut karna
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    for info in media_info_list:
        ...
        # Day + Episode format: "Day 03 - EP 04"
