...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # --- 3. CLEAN TITLE HANDLING (+ OR SYMBOL FIX & TV SHOW FILTERS) ---
    ...
    # TV Show filters: Day 01, Day 03 ya episode text ke aage ka sab cut karein
    day_match = re.search(r'\b(Day|D)\s*\d{1,3}\b.*$', title_to_clean, flags=re.IGNORECASE)
    if day_match:
        title_to_clean = title_to_clean[:day_match.start()].strip()

    # Brackets, quotes aur symbols clean karein
    ...
    # Random split numbers hatayein jaise "20 4"
    cleaned_title = re.sub(r'\b\d{1,2}\s+\d{1,2}\b', ' ', cleaned_title)
    
    # Symbols (+, -, &, |) ko sanitize karein
