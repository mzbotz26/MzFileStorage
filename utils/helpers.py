...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # ============================================================
    # 🎯 1. DIRECT SEASON & EPISODE EXTRACTION
    # ============================================================
    season_info_str = ""
    episode_info_str = ""

    # Season (e.g. S01, S1, Season 1)
    s_match = re.search(r'\b(?:S|Season)\s*0*(\d{1,2})\b', clean_name_ascii, re.IGNORECASE)
    if s_match:
        season_info_str = f"S{int(s_match.group(1)):02d}"

    # Episode Range: [E01 - E05], [E06 - E09], [E01-E05], [1-5], E01-E05
    range_match = re.search(
        r'\[?\s*(?:EP?|E)?\s*0*(\d{1,3})\s*(?:[-–—toTo]|\s+)\s*(?:EP?|E)?\s*0*(\d{1,3})\s*\]?',
        clean_raw_name,
        re.IGNORECASE
    )
    if range_match:
        s_ep, e_ep = int(range_match.group(1)), int(range_match.group(2))
        if 0 < s_ep < e_ep < 500:
            episode_info_str = f"EP {s_ep:02d}-{e_ep:02d}"

    # Single Episode: E01, EP01, Episode 1
    if not episode_info_str:
        single_match = re.search(r'\b(?:EP?|Episode)\s*0*(\d{1,3})\b', clean_name_ascii, re.IGNORECASE)
        if single_match:
            ep_num = int(single_match.group(1))
            if ep_num < 1000:
                episode_info_str = f"EP {ep_num}"

    # Day info (Reality shows)
    day_info_str = ""
    day_match = re.search(r'\b(?:Day|D)\s*0*(\d{1,3})\b', clean_name_ascii, re.IGNORECASE)
    if day_match:
        day_info_str = f"Day {int(day_match.group(1)):02d}"

    part_info = ""
    part_match = re.search(r'part[\s._-]?(\d+)', clean_name_ascii, re.IGNORECASE)
    if part_match:
    ...
    name_for_ptn = clean_name_ascii.replace('_', ' ').replace('.', ' ')
    name_for_ptn = re.sub(r'(?:www\.)?[\w-]+\.(?:com|org|net|xyz|me|io|in|cc|biz|world|info|club|mobi|press|top|site|tech|online|store|live|co|shop|fun|tamilmv)\b', '', name_for_ptn, flags=re.IGNORECASE)
    name_for_ptn = re.sub(r'@[a-zA-Z0-9_]+', '', name_for_ptn).strip()
    name_for_ptn = re.sub(r'\[.*?\]|\(.*?\)', ' ', name_for_ptn)
    ...
    if not initial_title:
        initial_title = name_for_ptn.split()[0] if name_for_ptn else "Video"

    # Remove junk and seasons from title base
    cleaned_title = re.sub(r'\b(?:S|Season)\s*\d{1,2}\b', '', initial_title, flags=re.IGNORECASE)
    cleaned_title = re.sub(r'\b(?:19\d{2}|20\d{2})\b', '', cleaned_title).strip()
    cleaned_title = re.sub(r'[-_.]', ' ', cleaned_title)

    junk_words = [
        'Ep', 'Eps', 'Episode', 'Episodes', 'Season', 'Series', 'South', 'Dubbed', 'Completed', 'Esub', 'Villa',
    ...
    cleaned_title = re.sub(r'\b(' + '|'.join(junk_words) + r')\b', ' ', cleaned_title, flags=re.IGNORECASE)
    cleaned_title = re.sub(r'^[^\w]+|[^\w]+$', '', cleaned_title).strip()
    cleaned_title = re.sub(r'\s+', ' ', cleaned_title).strip()
    
    final_title = cleaned_title.title() if cleaned_title else "File"

    # ============================================================
    # 🎯 3. YEAR & DISPLAY TITLE FORMATTING
    # ============================================================
    final_year = extracted_year or parsed_info.get('year')
    if not final_year:
        try:
            poster_data = await get_poster(final_title)
            if poster_data and isinstance(poster_data, tuple) and len(poster_data) >= 2:
                final_year = poster_data[1]
        except Exception:
            pass

    # Build Display Title: "Vikings S01 (2013)"
    ...
    if source_match:
        ...
        if "WEB" in s and "DL" in s: s = "WEB-DL"
        elif "WEB" in s and "RIP" in s: s = "WEBRip"
        elif "BLU" in s or "BR" in s or "BD" in s: s = "BluRay"
    ...
    if codec_match:
        extracted_tags.append(codec_match.group(1).upper().replace(' ', '.'))
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    media_info_list.sort(key=lambda x: (
        extract_first_number(x.get('day_info')),
        extract_first_number(x.get('episode_info')),
        extract_first_number(x.get('part_info'))
    ))
    ...
    for info in media_info_list:
        ...
        ep_text = info.get('episode_info', '') or ""
