...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # --- 1. PEHLE RANGE CHECK KAREIN (HANDLES: Ep 01 04, Ep 01-04, 01-04, 1 to 4) ---
    range_patterns = [
    ...
    for pattern, p_type in range_patterns:
        ...
        if match:
            ...
            if int(start_ep) < int(end_ep) and int(end_ep) < 1000:
                episode_info_str = f"EP {int(start_ep)}-{int(end_ep)}"
    ...
    if not episode_info_str:
        ...
        if ep_val:
            if isinstance(ep_val, list):
                if len(ep_val) > 1: episode_info_str = f"EP {int(min(ep_val))}-{int(max(ep_val))}"
    ...
    junk_words = [
        'Ep', 'Eps', 'Episode', 'Episodes', 'Season', 'Series', 'South', 'Dubbed', 'Completed', 'Esub',
        'Web', r'\d+Kbps', 'UNCUT', 'ORG', 'HQ', 'ESubs', 'MSubs', 'REMASTERED', 'REPACK', 'Hd', 'Hdm',
        'PROPER', 'iNTERNAL', 'Sample', 'Video', 'Dual', 'Audio', 'Multi', 'Hollywood', 'Hdtc',
