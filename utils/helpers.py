...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # 🛑 STEP 0: DS4K, 4K, UHD jaise tags ko sabse pehle clean karein
    ...
    # Standalone Season: 'S01' ya 'Season 1' - Strict separation
    s_match = re.search(r'(?i)(?:[_\.\-\s\[]|^)(?:Season\s*0*(\d{1,2})|S\s*0*(\d{1,2}))(?:[_\.\-\s\]]|$)', clean_name_ascii)
    if s_match:
    ...
    # PTN Fallback: Sirf tab maano jab filename me actually "Season X" ya "SX" present ho
