...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # DS4K aur 4K tags ko clean karein taaki 'S4' fake season na ban sake
    ...
    range_patterns = [
        # Underscores: _E01_E05_, _E06_E09_, -E01-E05-
        ...
        # Season with Episode Range: S01 E01-E05, S01 [E01-E05]
    ...
    # DS4K ko name_for_parsing se bhi remove karein taaki PTN isse Season 4 na maane
    ...
    # PTN Season Fallback: Tabhi accept karein jab filename me genuinely S ya Season pattern maujood ho
    if not season_info_str and parsed_info.get('season'):
        s_val = parsed_info.get('season')
        if re.search(rf'(?<![A-Za-z])(?:S|Season)\s*0*{s_val}\b', clean_name_ascii.replace('_', ' '), re.IGNORECASE):
            try:
    ...
    # Series hone par hi stray numbers saaf karein (Movie sequels like Jayanti 2 ko bachane ke liye)
    ...
    # Movie sequel retention (e.g. Jayanti 2)
    ...
    # ============================================================
    # 🔒 SINGLE-YEAR RESOLUTION (NO DUPLICATES)
    # ============================================================
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    # ============================================================
    # 🔒 BATCH-WIDE YEAR RESOLUTION (PREVENTS MISSING YEAR IN POST TITLE)
    # ============================================================
    batch_year = None
    ...
    # Agar pehli file me year missing tha to batch ke doosre files ka year use karein
    if batch_year and not re.search(r'\b(19\d{2}|20\d{2})\b', first_info['display_title']):
        primary_display_title = f"{first_info['display_title']} ({batch_year})"
    else:
    ...
    post_poster = await get_poster(poster_search_query, batch_year or first_info['year']) if user.get('show_poster', True) else None
    ...
    for info in media_info_list:
        ...
        ep_text = ""
        # Preserves episode range like EP 01-05, EP 06-09
