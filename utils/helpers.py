...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # --- 1. PEHLE RANGE CHECK KAREIN (PRIORITY) ---
    range_patterns = [
    ...
    if not episode_info_str:
        ...
        if direct_se:
            ...
            episode_info_str = f"EP {int(direct_se.group(2)):02d}"
    ...
    # Episode extraction fallback (sirf agar range aur direct ep dono na mile hon)
    if not episode_info_str:
