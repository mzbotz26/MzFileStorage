...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # Single-year formatted representation
    if final_year:
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    # Pehle se maujood koi bhi 4-digit year brackets saaf karein
    clean_base_title = re.sub(r'[\(\[]\s*(?:19\d{2}\vert{}20\d{2})\s*[\)\]]', '', first_info['display_title']).strip()
    clean_base_title = re.sub(r'\s+', ' ', clean_base_title).strip()

    final_post_year = batch_year or first_info.get('year')
    if final_post_year:
        primary_display_title = f"{clean_base_title} ({final_post_year})"
    else:
        primary_display_title = clean_base_title
