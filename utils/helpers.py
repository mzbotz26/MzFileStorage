...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # Unicode normalization: stylized mathematical letters ko standard text me convert ya remove karna
    normalized_name = unicodedata.normalize('NFKD', name)
    clean_name_ascii = normalized_name.encode('ascii', 'ignore').decode('ascii')
    
    # Telegram channels aur handles remove karein
    ...
    # Reality shows ke Day tag ko detect karna (e.g. Day.03, Day 3)
    day_info_str = ""
    ...
    # Direct SXXEXXX check (Supports big episodes like S09E854)
    ...
    # Episode extraction fallback (Supports large integers)
    ...
    # TV Shows tagline strip (e.g., 'Comedycha 5G', 'Day 03', 'OTT')
    title_to_clean = re.sub(r'\b(comedycha\s*5g|comedycha)\b.*$', '', title_to_clean, flags=re.IGNORECASE)
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    for info in media_info_list:
        ...
        # Deduplication check for quality tags
