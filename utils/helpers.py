...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # 1. Filename se 4-digit release year pakadein
    ...
    # ============================================================
    # 🎯 1. DIRECT EPISODE RANGE EXTRACTION (BEFORE CLEANING UNDERSCORES)
    # ============================================================
    season_info_str = ""
    episode_info_str = ""
    ...
    # Range check patterns: _E01_E05_, [E01 - E05], E01-E05, 01-05
    range_patterns = [
        # ✅ Handles: _E01_E05_, _E06_E09_, -E01-E05-
        (r'(?:[_\.\-\s\[]|^)(?:EP?|E)?\s*0*(\d{1,3})\s*[_\.\-\s]+(?:EP?|E)\s*0*(\d{1,3})(?:[_\.\-\s\]]|$)', 'no_season'),
        # ✅ Handles: [E01 - E05], [01 - 05]
        (r'\[\s*(?:EP?|E)?\s*0*(\d{1,3})\s*(?:[-–—toTo]|\s+)\s*(?:EP?|E)?\s*0*(\d{1,3})\s*\]', 'no_season'),
        # ✅ Handles: S01 E01-E05, S01 [E01-E05]
    ...
    # Single episode fallback agar range nahi mili
    ...
    # ============================================================
    # 🔒 BULLETPROOF YEAR LOCK (NO LOSS, NO DUPLICATES)
    # ============================================================
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    for info in media_info_list:
        ...
        ep_text = ""
        # ✅ Preserves range: EP 01-05, EP 06-09
