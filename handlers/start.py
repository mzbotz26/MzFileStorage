...
async def send_file(client, requester_id, owner_id, file_unique_id):
    try:
        ...
        # ✅ Sahi DB Channel message ID assign kiya
...
async def handle_public_file_request(client, message, requester_id, payload):
    ...
    # ===============================
    # VERIFY CHECK (KOYEB PROTECTION TIMER)
    # ===============================
    ...
    if not verified:
        ...
        code_string = f"verify_{owner_id}_{file_unique_id}"
        encoded_string = base64.urlsafe_b64encode(code_string.encode("ascii")).decode("ascii").strip("=")
        
        # 2. Koyeb Protection Link (5-Sec Timer)
        koyeb_verify_url = f"{Config.APP_URL.rstrip('/')}/{encoded_string}"
        ...
        buttons = [
            [InlineKeyboardButton("🔐 Verify Now", url=koyeb_verify_url)],
