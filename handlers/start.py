...
async def send_file(client, requester_id, owner_id, file_unique_id):
    try:
        ...
        # ✅ Crash Guard: sent_message null check
...
async def handle_public_file_request(client, message, requester_id, payload):
    ...
    if not verified:
        ...
        if not has_active_shortener:
            ...
            # 🔴 SHORTENER ON: HMAC-SHA256 Signed Koyeb Destination URL
            ts = int(time.time())
            protect_payload = f"{owner_id}_{file_unique_id}_{requester_id}"
            raw_data = f"{protect_payload}_{ts}"
            ...
            koyeb_destination = f"{Config.APP_URL.rstrip('/')}/destination/{protect_payload}?sig={sig}___{ts}"
